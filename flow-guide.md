...
완료 후 GitHub URL, Vercel URL을 제가 `project/info.md`에 **자동으로 저장**합니다.

---

## 2. 프로젝트 정보 확인 (1번 완료 후)

1번을 AI가 직접 실행했다면 `project/info.md`가 이미 채워져 있습니다.
**"info.md 확인해 보시고 맞으면 OK 해주세요."** 라고 안내하면 됩니다.

수동으로 직접 진행하신 경우: 프로젝트명, GitHub URL, Vercel URL을 채팅으로 알려주세요. 제가 info.md에 반영합니다.
...
- [ ] 1. GitHub 레포 + Vercel 배포 → URL 자동 저장
- [ ] 2. info.md 확인 (자동 완료 or 수동 입력)
...
언제든 "N번으로 돌아가줘" 또는 "PRD 수정하고 싶어"처럼 말하면 됩니다.
제가 해당 단계로 되돌아가서 다시 진행합니다.

- "PRD 수정하고 싶어" → 4번으로 돌아가 재작성
- "DB 설계 다시 해줘" → 6번으로 돌아가 재설계
- "시안 새로 올릴게" → 3번으로 돌아가 재분석
- "API 키 바뀌었어" → required-info.md만 바로 업데이트 (단계 변경 없음)

기존 산출물이 있으면 덮어쓰기 전에 한 번 확인합니다.
